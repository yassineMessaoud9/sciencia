...
export default {
    ...
    computed: {
        order: function () {
            return this.$store.getters['activeOrder/show'];
        },
        orderProducts: function () {
            return this.$store.getters['activeOrder/orderProducts'];
        },
        orderUser: function () {
            return this.$store.getters['activeOrder/orderUser'];
        },
        orderAddress: function () {
            return this.$store.getters['activeOrder/orderAddress'];
        },
        outletAddress: function () {
            return this.$store.getters['activeOrder/outletAddress'];
        },
    },
    mounted() {
        this.loading.isActive = true;
        this.$store.dispatch('activeOrder/show', this.$route.params.id).then(res => {
        ...
        changeStatus: function (status, amount) {
            if (status === paymentStatusEnum.UNPAID) {
                appService.confirmOrder(amount).then((res) => {
                    try {
                        this.loading.isActive = true;
                        this.$store.dispatch("activeOrder/changeStatus", {
