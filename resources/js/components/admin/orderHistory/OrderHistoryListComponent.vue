    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">{{ $t('menu.order_history') }}</h3>
                <div class="db-card-filter">
                    <TableLimitComponent @click.prevent="handlePaper" :method="list" :search="props.search"
                        :page="paginationPage" />
                    <FilterComponent @click.prevent="handleSlide('order-history-filter')" />
            ...
            <div class="table-filter-div" id="order-history-filter">
                ...
                <table class="db-table stripe" id="print">
                    <thead class="db-table-head">
                        <tr class="db-table-head-tr">
                            ...
                            <th class="db-table-head-th hidden-print" v-if="permissionChecker('order-history')">
                                {{ $t('label.action') }}
                            </th>
                        </tr>
                    </thead>
                    <tbody class="db-table-body" v-if="orders.length > 0">
                        <tr class="db-table-body-tr" v-for="order in orders" :key="order">
                            ...
                            <td class="db-table-body-td hidden-print" v-if="permissionChecker('order-history')">
                                <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                    <SmIconViewComponent :link="'admin.orderHistory.show'" :id="order.id"
                                        v-if="permissionChecker('order-history')" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
export default {
    name: "OrderHistoryListComponent",
    components: {
        LoadingComponent,
        PaginationSMBox,
        PaginationBox,
    ...
    data() {
        return {
            ...
            printObj: {
                id: "print",
                popTitle: this.$t("menu.order_history"),
            },
            enums: {
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('orderHistory/lists', this.props.search).then(res => {
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store.dispatch("orderHistory/export", this.props.search).then((res) => {
                this.loading.isActive = false;
                ...
                link.download = this.$t("menu.order_history");
                link.click();
