    ...
    <div id="modal" class="modal">
        <div class="modal-dialog">
            <div class="modal-header">
                <h3 class="modal-title">{{ $t('menu.countries') }}</h3>
...
export default {
    ...
    computed: {
        addButton: function () {
            return { title: this.$t("button.add_country") }
        }
    },
    mounted() {
    ...
    methods: {
        reset: function () {
            ...
            this.$store.dispatch('country/reset').then().catch();
            this.errors = {};
        ...
        save: function () {
            try {
                const tempId = this.$store.getters['country/temp'].temp_id;
                this.loading.isActive = true;
                this.$store.dispatch('country/save', {
                    form: this.props.form,
                    search: this.props.search
                }).then((res) => {
                    composables.closeModal('modal');
                    this.loading.isActive = false;
                    alertService.successFlip((tempId === null ? 0 : 1), this.$t('menu.countries'));
                    this.props.form = {
                        name: "",
                        code: "",
                        status: statusEnum.ACTIVE,
