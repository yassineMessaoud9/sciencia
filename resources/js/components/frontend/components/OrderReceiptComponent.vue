            ...
            <table>
                <tbody>
                    ...
                    <tr v-if="order.transaction && order.edited_amount !== 0">
                        ...
                        <td v-else class="pt-1 pb-1">{{ $t('label.return_amount') }} {{ order.edited_currency_amount }}
                        </td>
                    </tr>
                    <tr v-if="order.reason && parseInt(order.status) === parseInt(enums.orderStatusEnum.RETURNED)">
